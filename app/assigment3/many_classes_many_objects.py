...
def yolo_loss_function(
    ...
    obj_loss = F.binary_cross_entropy(
        pred_obj[obj_mask], target_box[..., 0][obj_mask], reduction="sum"
    )
    noobj_loss = F.binary_cross_entropy(
        pred_obj[noobj_mask], target_box[..., 0][noobj_mask], reduction="sum"
    )
    ...
    if indices.numel() > 0:
        ...
        decoded_preds = torch.stack(
            [decoded_cx, decoded_cy, decoded_w, decoded_h], dim=1
        )

...
# Use three classes: 0 for Background, 1 for Face, 2 For
num_classes = 3
...
class FaceDataset(Dataset):
    def __init__(self, img_dir, labels_dir, transform):
        ...
        for label_filename in os.listdir(labels_dir):
            ...
            # Get the base name (without extension) and form the corresponding image path.
            base_name = os.path.splitext(label_filename)[0]
            image_path = os.path.join(img_dir, base_name + ".jpg")  # Change the extension if needed
            ...
            for line in lines:
                ...
                if not line:
                    continue
                # Expecting the line format: class center_x center_y width height
                parts = line.split()
                if len(parts) != 5:
...
if __name__ == "__main__":
    ...
    train_dataset = FaceDataset(
        "many_face_dataset/images/train",
        "many_face_dataset/labels/train",
        transform)

...
# ---------------------------
# Webcam Inference and Recording
# ---------------------------
cap = cv2.VideoCapture(0)

# Read the first frame to get dimensions and initialize VideoWriter
ret, frame = cap.read()
if not ret:
...
# Attempt to get the camera FPS; if not available, fallback to 20.0 FPS.
fps = 15
...
out_writer = cv2.VideoWriter("output_video.mp4", fourcc, fps, (orig_w, orig_h))
...
while ret:
    ...
    # Resize frame to 400x400 for model input
    ...
    # If more than one detection is found, select only the one with the smaller area
    if final_detections.shape[0] > 1:
        areas = (final_detections[:, 2] - final_detections[:, 0]) * (
            final_detections[:, 3] - final_detections[:, 1]
        )
        best_idx = np.argmin(areas)
        final_detections = final_detections[best_idx : best_idx + 1]

    # Scale detections from 400x400 back to original frame dimensions

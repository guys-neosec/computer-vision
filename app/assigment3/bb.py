    ...
    def __init__(self, img_dir, ann_file, transform):
        ...
        for img_id, anns in annotations.items():
            if len(anns) > 0:
                ...
                labels = [ann.get("category_id", 0) for ann in anns]
                self.data.append((img_info, bboxes, labels))
...
def build_target(bboxes, labels, grid_h, grid_w, batch_size, device, anchors, num_anchors, num_classes):
    ...
    for i in range(batch_size):
        for face, lab in zip(bboxes[i], labels[i]):
            ...
            target_cls[i, cell_y, cell_x, best_anchor] = lab
...
def compute_loss(output, bboxes, labels, device, anchors, num_classes, lambda_bbox=1.0, lambda_cls=1.0):
    ...
    target_box, target_cls = build_target(bboxes, labels, grid_h, grid_w, batch_size, device, anchors, num_anchors, num_classes)
    pred_obj = output[..., 0]
    ...
    for b in range(batch_size):
        inds = (target_box[b, ..., 0] == 1).nonzero(as_tuple=False)
        for idx in inds:
            ...
            gt_cls = target_cls[b, gy, gx, anchor_idx]
            pred_cls = pred[5:]
            cls_loss_total += F.cross_entropy(pred_cls.unsqueeze(0), gt_cls.unsqueeze(0))
            count += 1
    if count > 0:
        giou_loss = giou_loss_total / count
        cls_loss = cls_loss_total / count
    else:
        giou_loss, cls_loss = 0.0, 0.0
...
def train(model, dataloader, optimizer, scheduler, epochs, device, writer, freeze_epochs=5):
    ...
    for epoch in range(epochs):
        ...
        logger.info(f"Epoch {epoch+1}/{epochs}, Total Loss: {avg_loss:.4f}, Obj Loss: {avg_obj:.4f}, Giou Loss: {avg_giou:.4f}, Cls Loss: {avg_cls:.4f}, LR: {current_lr:.6f}")
...
if __name__ == "__main__":
    ...
    train(model, train_loader, optimizer, scheduler, epochs, device, writer, freeze_epochs=5)

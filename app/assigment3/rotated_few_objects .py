...
class FaceDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for b in transformed["bboxes"]:
            ...
            bh = b[3] / H
            angle = b[4]
            norm_bboxes.append([cx, cy, bw, bh,angle])
...
class YOLOFaceDetector(nn.Module):
    def __init__(self, num_anchors, num_classes):
        ...
        self.head = YOLOHead(256, num_anchors * (6 + num_classes))
...
def build_target(bboxes, labels, grid_h, grid_w, batch_size, device, anchors, num_anchors, num_classes):
    ...
    target_box = torch.zeros((batch_size, grid_h, grid_w, num_anchors, 6), device=device)
...
def probiou(obb1, obb2, eps=1e-7):
    ...
    t3 = ((a1 + a2) * (b1 + b2) - (c1 + c2).pow(2)) / (4 * ((a1 * b1 - c1.pow(2)).clamp_(0) * (a2 * b2 - c2.pow(2)).clamp_(0)).sqrt() + eps).log() * 0.5
...
def compute_loss(output, bboxes, labels, device, anchors, num_classes, lambda_bbox=1.0, lambda_cls=1.0):
    ...
    for b in range(batch_size):
        ...
        for idx in inds:
            ...
            gt_angle = t[5]
            gt_box = torch.stack([gt_cx.detach(), gt_cy.detach(), gt_w_tensor, gt_h_tensor],gt_angle.detach())
            obb_loss_total += probiou(pred_box, gt_box)
            count += 1
    obb_loss = obb_loss_total / count if count > 0 else 0.0
    ...
    total_loss = loss_obj + lambda_bbox * obb_loss + lambda_cls * cls_loss
    return total_loss, loss_obj, obb_loss, cls_loss
...
if __name__ == "__main__":
    transform = A.Compose([
        ...
        ToTensorV2()
    ], bbox_params=A.BboxParams(format="obb", label_fields=["labels"]))

    train_dataset = FaceDataset(
        "app/assigment3/Chess-Pieces-Detection-1/train",
        "app/assigment3/Chess-Pieces-Detection-1/train/_annotations.coco.json",
        transform)
